...
//    println!("{:?}",txt.text1)
    let vec = WordsVector::vocab_of_sentences(&txt);
    let vec = vec.replace_by_u2581();
...
//    let vec = WordsVector::string_toend(vec,"\x20\x20</w>\x20\x20");
    ...
    println!("The initial tokens correspond to the unicode scalars : chars, except  end of word");
...
//          tokens = VocabOfTokens::from_words_vocab_bpe(&vocab);
    ...
    tokens_size = tokens.tokens.keys().len();
    println!("Number of program calculated tokens {}", tokens_size);
